...
import BitableEditor from "@/components/BitableEditor";
import ContentMentionShell from "@/components/ContentMentionShell";
import { useAuth } from "@/components/AuthProvider";
import type { Document } from "@/types/document";
import type { BitableData } from "@/lib/content-types";
...
export default function BitablePage() {
  ...
  const [content, setContent] = useState<BitableData | null>(null);
  const [loading, setLoading] = useState(true);
  const [editorKey, setEditorKey] = useState(id);

  const documentHref = getContentPath(id, "bitable");
  ...
  const applyRemoteDocument = useCallback(
    (remote: Document) => {
      if (remote.contentType !== "bitable") return;
      setDoc(remote);
      setTitle(remote.title);
      syncTitleMeta(remote.title, remote.updatedAt);
      setContent(remote.content as unknown as BitableData);
  ...
  useEffect(() => {
    async function load() {
      ...
      setContent(data.content as unknown as BitableData);
      dirtyRef.current = false;
      setLoading(false);
    }
    load();
